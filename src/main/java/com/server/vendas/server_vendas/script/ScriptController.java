...
public class ScriptController {
  ...
  @PostMapping
  public ResponseEntity<ScriptDto> create(ScriptDto scriptDto) {
    return ResponseEntity.status(HttpStatus.OK).body(this.scriptService.save(scriptDto));
  }

  @GetMapping
  public ResponseEntity<FindAllScriptDto> findAll() {
    return ResponseEntity.status(HttpStatus.OK).body(this.scriptService.findAll());
  }

  @GetMapping("{id}")
  public ResponseEntity<ScriptDto> findById(@PathVariable @Valid UUID id) {
    return ResponseEntity.status(HttpStatus.OK).body(this.scriptService.findById(id));
  }

  @PatchMapping("{id}")
  public ResponseEntity<ScriptDto> update(
      @PathVariable @Valid UUID id, @RequestBody @Valid ScriptDto scriptDto) {
    return ResponseEntity.status(HttpStatus.OK).body(this.scriptService.update(id, scriptDto));
  }
